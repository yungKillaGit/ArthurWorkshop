...
const servicesDB = require('../../services-db');
...
const getRequest = async (id) => {
  ...
  return Request
    ...
    .then((response) => {
      ...
      return { status: 200, data: map(response) };
    });
};

const addRequest = async (requestInfo) => {
  ...
  return Request
    ...
    .then((response) => {
      ...
      return populatedRequest.then((doc) => ({ status: 201, data: map(doc) }));
    });
};
...
const updateRequest = async (id, requestInfo) => {
  const validationResult = errorHandler.checkIfIdIsValid(id);
  if (validationResult instanceof Error) {
    throw validationResult;
  }
